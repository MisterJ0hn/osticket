...
class DraftAjaxAPI extends AjaxController {
    ...
    function createDraftClient($namespace) {
        ...
        if (!$thisclient && substr($namespace, -12) != substr(osTicketSession::getStableId(), -12))
    ...
    function getDraftClient($namespace) {
        ...
        if ($thisclient) {
            ...
            if (substr($namespace, -12) != substr(osTicketSession::getStableId(), -12))
    ...
    function updateDraftClient($id) {
        ...
        // stable session id for others
        elseif ($thisclient) {
            if ($draft->getStaffId() != $thisclient->getId())
                Http::response(404, "Draft not found");
        }
        else {
            if (substr(osTicketSession::getStableId(), -12) != substr($draft->getNamespace(), -12))
                Http::response(404, "Draft not found");
        }
