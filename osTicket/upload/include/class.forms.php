...
class FormField {
    ...
    function getFormName() {
        ...
        // NOTE: osTicketSession::getStableId() is used here instead of
        // session_id() because session_id() can rotate mid-request-cycle
        // for logged-in clients (session fixation mitigation - see
        // UserSession::regenerateSession()). If that happens between the
        // time this field is rendered and the time its containing form is
        // submitted/autosaved, a session_id()-based hash would no longer
        // match what's embedded in the already-rendered page, and the
        // field would appear to go missing from the request.
        if ($this->_form && is_numeric($fid = $this->_form->getFormId()))
            return substr(md5(
                osTicketSession::getStableId() . "-form-field-id-$fid-$default-" . SECRET_SALT), -14);
        elseif (is_numeric($this->get('id')))
            return substr(md5(
                osTicketSession::getStableId() . '-field-id-'.$this->get('id') . '-' . SECRET_SALT), -16);
    ...
    function render($options=array()) {
        ...
        if ($options['client']) {
            $namespace = $options['draft-namespace']
                ?: 'ticket.client';
             // Use a stable, session-persisted id rather than raw
             // session_id() -- the latter can rotate mid-request-cycle for
             // logged-in clients (see UserSession::regenerateSession()),
             // which would silently invalidate the draft namespace baked
             // into the already-rendered page.
             $object_id = substr(osTicketSession::getStableId(), -12);
        } else {
