...
class osTicketSession {
    ...
    static function regenerate(int $ttl=60) {
        ...
        session_start();
        // Now that the id has actually changed, push the *new* session id
        // to the browser. Doing this before session_regenerate_id() (as
        // before) sent the cookie with the *old* id, so the browser kept
        // presenting the old (soon to expire) session on every subsequent
        // request - once it expired, session data (including the CSRF
        // token) reset silently, causing "Invalid CSRF Token" errors.
     ...
     * getStableId
     *
     * Returns an identifier that stays constant for the life of the
     * session, even across a session_id() rotation performed by
     * ::regenerate() (session fixation mitigation - see
     * UserSession::regenerateSession()). session_id() itself is NOT safe
     * to bake into rendered pages (e.g. draft namespaces, dynamic form
     * field name hashes) because a logged-in client's id can rotate
     * mid-request-cycle, silently invalidating anything derived from the
     * old value. $_SESSION data survives the rotation, so a token stored
     * there does not.
     */
    static function getStableId() {
        if (empty($_SESSION['_stable_id']))
            // Restricted to a word-char alphabet (no `=`, which
            // Misc::randCode() includes by default) since this value is
            // embedded in draft namespace URLs matched against a `\w`
            // route regex in ajax.php.
            $_SESSION['_stable_id'] = Misc::randCode(32,
                'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789');

        return $_SESSION['_stable_id'];
    }
